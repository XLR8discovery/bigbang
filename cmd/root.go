...
import (
	...
	"xlr8d.io/bigbang/pkg/runtime/compose"
...
var RootCmd = &cobra.Command{
	...
	Long: `bigbang can be used to create containerized or standalone runtime environment,
leverages existing binaries, or even references code to be built in docker and create images.

For example:

bigbang build remote gerrit 5826`,
}

// Execute is the execution of the top level bigbang command.
func Execute() {
	err := RootCmd.Execute()
	if err != nil {
		log.Fatalf("%++v", err)
...
// ExecuteBigBang can execute any operation with loaded stack/runtime and write back the results.
func ExecuteBigBang(exec func(stack recipe.Stack, rt runtime.Runtime, args []string) error) func(cmd *cobra.Command, args []string) error {
